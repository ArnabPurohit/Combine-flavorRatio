...
void data_fit(){
  ...
  TFile *bin_file = new TFile("templates/tmp2016bb_multiBins.root","READ");
  ...
  TFile *inputfile = new TFile("workspace_bb.root","READ");
  ...
  */
  ...
  RooAddPdf *model = new RooAddPdf("model", "g1+g2+a", RooArgList(*sig_model_1, *sig_model_2, *sig_model_3, *sig_model_4, *sig_model_5, *sig_model_6, *sig_model_7, *sig_model_8, *sig_model_9), RooArgList(*nsig1, *nsig2, *nsig3, *nsig4, *nsig5, *nsig6, *nsig7, *nsig8));
  model->fitTo(*dataset);
  
  //sig_model_3->plotOn(xframe,  Range("full"), RooFit::NormRange("full"));
  //RooAbsReal* i = sig_model_3->createIntegral(*mass);
  model->plotOn(xframe,  Range("full"), RooFit::NormRange("full"));
